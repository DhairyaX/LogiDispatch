...
def print_config_summary(
    ...
    table.add_row("Balancing Mode", settings.optimizer.balancing_mode.value.upper())
...
def print_metrics(metrics: Metrics) -> None:
    ...
    table.add_row("Balancing Mode", metrics.balancing_mode.upper())
    ...
    if metrics.vehicle_count > 1:
        ...
        table.add_row("Fleet Utilization", f"{metrics.vehicle_utilization_rate}%")
    ...
    if getattr(metrics, "balancing_mode", "distance_optimal") != "distance_optimal" and metrics.vehicle_count > 1:
        table.add_row("", "")
        table.add_row("[bold bright_cyan]--- Workload Distribution ---[/]", "")
        mean_stops = metrics.total_stops / metrics.vehicles_used if metrics.vehicles_used > 0 else 0
        table.add_row("Average Stops", f"{mean_stops:.1f}")
        table.add_row("Maximum Stops", str(metrics.max_stops_per_vehicle))
        table.add_row("Minimum Stops", str(metrics.min_stops_per_vehicle))
        table.add_row("Workload Std Dev", f"{metrics.stop_distribution_stddev:.2f}")
