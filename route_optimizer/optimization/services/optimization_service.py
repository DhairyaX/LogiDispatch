...
class OptimizationService:
    ...
    def solve_vrp(
        ...
        # ── 3b. Workload Balancing (Count, Distance, Duration) ──
        if effective_vehicles > 1 and settings.optimizer.balance_workload:
            dist_weight = 0
            dur_weight = 0
            count_weight = 0

            if balancing_mode == "strict":
                dist_weight = balance_weight * 10000
                dur_weight = balance_weight * 10000
                count_weight = balance_weight * 10000
            elif balancing_mode == "balanced":
                dist_weight = balance_weight * 500
                dur_weight = balance_weight * 500
            elif balancing_mode == "distance_optimal":
                # Only softly balance the primary objective metric
                if optimization_mode == "distance":
                    dist_weight = balance_weight * 100
                elif optimization_mode == "duration":
                    dur_weight = balance_weight * 100

            # 1. Stop Count Balancing
            if count_weight > 0:
                routing.AddConstantDimension(
                    1,  # increment by 1
                    num_locations,  # max stops per vehicle
                    True,  # start cumul to zero
                    "Count",
                )
                count_dimension = routing.GetDimensionOrDie("Count")
                count_dimension.SetGlobalSpanCostCoefficient(count_weight)

            # 2. Distance Balancing
            if dist_weight > 0:
                # We use the existing transit_callback_index for distance
                routing.AddDimension(
                    transit_callback_index,
                    0,  # slack max
                    30000000,  # capacity (very large)
                    True,  # start cumul to zero
                    "Distance",
                )
                distance_dimension = routing.GetDimensionOrDie("Distance")
                distance_dimension.SetGlobalSpanCostCoefficient(dist_weight)

            # 3. Duration Balancing (if available)
            if dur_weight > 0 and duration_matrix is not None:
                def _duration_callback(from_index: int, to_index: int) -> int:
                    from_node = manager.IndexToNode(from_index)
                    to_node = manager.IndexToNode(to_index)
                    return duration_matrix[from_node][to_node]

                duration_callback_index = routing.RegisterTransitCallback(_duration_callback)
                routing.AddDimension(
                ...
                duration_dimension.SetGlobalSpanCostCoefficient(dur_weight)
